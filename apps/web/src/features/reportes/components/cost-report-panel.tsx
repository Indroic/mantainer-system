  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="relative overflow-hidden border-slate-800/80 bg-slate-900/40 backdrop-blur-md rounded-2xl shadow-xl">
          ...
          <CardContent className="p-4 pt-0 flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-400">Inversión acumulada en materiales de mantenimiento</p>
            </div>
        ...
        <Card className="relative overflow-hidden border-slate-800/80 bg-slate-900/40 backdrop-blur-md rounded-2xl shadow-xl">
          ...
          <CardContent className="p-4 pt-0 flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-400">Máquinas que requirieron recambios físicos</p>
            </div>
        ...
        <Card className="relative overflow-hidden border-slate-800/80 bg-slate-900/40 backdrop-blur-md rounded-2xl shadow-xl">
          ...
          <CardContent className="p-4 pt-0 flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-400">Máximo consumo registrado: {breakdown[0]?.machine_code || "N/A"}</p>
            </div>
                  ...
                  <div key={item.machine_code} className="space-y-1.5 group">
                    <div className="flex justify-between items-center text-xs font-semibold">
                      ...
                      <div className="flex items-center gap-3 font-mono">
                        <span className="text-slate-300">${item.spare_parts_cost.toFixed(2)}</span>
                        <span className="text-[10px] text-indigo-400">({percentage.toFixed(1)}%)</span>
                      </div>
                    </div>

                    {/* Barra de Progreso Premium SVG NATIVA */}
                    <div className="relative h-3 w-full rounded-full bg-slate-950 overflow-hidden border border-slate-850">
                      <div
