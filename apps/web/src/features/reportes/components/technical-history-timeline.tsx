...
export default function TechnicalHistoryTimeline({ orders }: TechnicalHistoryTimelineProps) {
  ...
  if (liquidatedOrders.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-8 bg-surface/20 border border-border rounded-2xl text-center">
        <ClipboardListIcon className="size-8 text-slate-600 mb-2 animate-pulse" />
        <p className="text-muted-foreground text-sm font-semibold">Sin registros en la bitácora técnica de mantenimientos.</p>
      ...
      {liquidatedOrders.map((order) => {
        ...
        return (
          <div key={order.id} className="relative group">
            ...
            <span className="absolute -left-[31px] md:-left-[39px] top-1.5 flex size-6 items-center justify-center rounded-full bg-background border border-border group-hover:border-indigo-500 transition-all duration-300">
              <span className="size-2 rounded-full bg-indigo-500 animate-pulse" />
            </span>

            {/* Contenedor Principal de la Tarjeta */}
            <Card className="bg-surface/20 border-border hover:border-indigo-500/30 transition-all duration-300 rounded-2xl overflow-hidden shadow-xl">
              <CardHeader className="p-4 pb-2 border-b border-border bg-background/80 flex flex-row items-center justify-between">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-2">
                    <WrenchIcon className="size-4 text-indigo-400" />
                    <CardTitle className="text-sm font-bold text-foreground">{order.description}</CardTitle>
                  </div>
              ...
              <CardContent className="p-4 space-y-4">
                  ...
                  {order.spare_parts.length === 0 ? (
                    ...
                    <div className="overflow-x-auto rounded-xl border border-border bg-background/80">
                      <table className="w-full text-xs font-medium text-left">
                        <thead>
                          <tr className="border-b border-border bg-background/80 text-muted-foreground text-[10px] uppercase font-bold">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3.5 bg-background/80 border border-border rounded-xl gap-3">
                  {canSeeFinancials ? (
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      <WrenchIcon className="size-4 text-indigo-400" />
                      <div>
                        <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">Detalles de la Intervención</p>
                        <p className="text-xs text-foreground/80">Repuestos reemplazados correctamente</p>
                      </div>
                    </div>
                  )}
